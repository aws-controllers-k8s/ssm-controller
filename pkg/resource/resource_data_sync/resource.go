...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.SyncName = &identifier.NameOrID

	f2, f2ok := identifier.AdditionalKeys["syncType"]
	if f2ok {
		r.ko.Spec.SyncType = aws.String(f2)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["syncName"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: syncName"))
	}
	r.ko.Spec.SyncName = &primaryKey

	f2, f2ok := fields["syncType"]
	if f2ok {
		r.ko.Spec.SyncType = aws.String(f2)
	}
